...
@Tag(
        name = "CRUD REST APIs for Loans in KanapBank",
        description = "CRUD REST APIs in KanapBank to CREATE, UPDATE, FETCH AND DELETE loan details"
)
@RestController
...
public class LoansController {
    ...
    @Operation(
            ...
            description = "REST API to create new loan inside KanapBank"
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "201",
    ...
    @GetMapping
    public ResponseEntity<LoansDto> fetchLoanDetails(@RequestHeader("kanap-correlation-id") String correlationId,
                                                     @RequestParam
                                                     @Pattern(regexp="(^$|[0-9]{10})",message = "Mobile number must be 10 digits")
                                                     String mobileNumber) {
